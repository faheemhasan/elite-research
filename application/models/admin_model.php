...
class Admin_model extends CI_Model {	
	...
	public function get_neighbour_row($table_name='', $id='',$from="")
	{
	...
	public function get_result($table_name='', $id_array='',$id_array2='', $limit=''){
		if(!empty($id_array)):		
	...
	public function get_order_result($table_name='',$order=''){
	...
	public function get_under_header(){
		$this->db->order_by('order', 'asc');
		$query=$this->db->get('under_header');
		if($query->num_rows()>0)
			return $query->result();
		else
			return FALSE;
	...
	public function get_limited_results($table_name='', $limit='', $id_array=''){
		if(!empty($id_array)):		
			foreach ($id_array as $key => $value){
				$this->db->where($key, $value);
			}
