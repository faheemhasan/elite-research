...
*/
	public function get_rules_create($post)
	{
		...
		foreach($table_fields as $fname)
		{
			...
			$rules_array['1'] = 'required';
 			...
 			$rules_array['100'] = "callback_image_validation_for_create['.@$"."_FILES['".$fname."']['name'].']"; 
			$rules_parameter = '';
	...
	public function get_rules_update($post)
	{
		...
		foreach($table_fields as $fname)
		{
			...
			$rules_array['1'] = 'required';
	 		...
	 		$rules_array['11'] = 'integer';
	 		$rules_array['12'] = 'decimal'; 
 			$rules_array['100'] = "callback_image_validation_for_update['.@$"."_FILES['".$fname."']['name'].']"; 
			$rules_parameter = '';
	...
	public function get_insert_array($post)
	{
		...
		foreach($table_fields as $fname)
		{
			...
			{
				$data_string .=	"	'".strtolower($fname)."' => $".strtolower($fname).",";
			}
			else
			{
	...
	public function image_data_create($post)
	{
		...
		foreach($fname as $fname)
		{
			...
			if($ftype == 'file')
			{
				...
				$image_data .= "				$".$fname." = uniqid().'.'.$"."ext;\n";
				$image_data .= "   				move_uploaded_file($"."_FILES['".$fname."']['tmp_name'],'./assets/uploads/".$module."/'.$".$fname.");\n";			
				$image_data .= "   				create_thumb($".$fname.",'./assets/uploads/".$module."/');\n";
				$image_data .= "			}\n\n";
			}
			$i++;
		}
	...
	public function create_controller($post)
	{
		...
		if($status)
		{
			...
			$content .= "	}\n\n";
			$content .=	"	public function image_validation_for_create($"."image_not='0',$"."image)\n";
			$content .= "  	{\n";		
			$content .= "		if($"."image==''){\n";
			$content .= "			$"."this->form_validation->set_message('image_validation_for_create','Please select an image to upload.');\n";
			$content .= " 			return false;\n";
			$content .= "		}else{\n";
			$content .= "			$"."image = explode('.',$"."image);\n";
			$content .= "			$"."format = $"."image[count($"."image)-1];\n";
			$content .= "			$"."format = strtolower($"."format);\n";
			$content .=	"			if($"."format == 'png' || $"."format == 'jpeg' || $"."format == 'jpg' || $"."format == 'gif'){\n";
			$content .= "				return true;\n";
			$content .= "			}else{\n";
			$content .=	"				$"."this->form_validation->set_message('image_validation_for_create','Image format not allowed.');\n";
			$content .=	"				return false;\n";
			$content .= "			}\n";
			$content .= "		}\n";
			$content .=	"	}\n\n";
			$content .=	"	public function image_validation_for_update($"."image_not='0',$"."image)\n";
			$content .= "  	{\n";		
			$content .= "		if($"."image==''){\n";
			$content .= " 			return true;\n";
			$content .= "		}else{\n";
			...
			$content .= "				return true;\n";
			$content .= "			}else{\n";
			$content .=	"				$"."this->form_validation->set_message('image_validation_for_update','Image format not allowed.');\n";
			$content .=	"				return false;\n";
			$content .= "			}\n";
			$content .= "		}\n";
			$content .=	"	}\n\n";
...
*/
	...
	public function write_create_view($post)
	{
		...
		$content .= "<input type='submit' class='btn btn-primary' value='Add'>\n";
		$content .= "</div>\n";
		$content .= "</div>\n";
	...
	public function write_update_view($post)
	{
		...
		$content .= "<div class='form-group'>\n";
		$content .= "<div class='col-lg-offset-2 col-lg-10'>\n";
		$content .= "<input type='submit' class='btn btn-primary' value='Edit'>\n";
		$content .= "</div>\n";
		$content .= "</div>\n";
		$content .= "<?php echo form_close(); ?>\n";
		$content .= "</div>\n";
	...
	public function write_all_view($post)
	{
		...
		$content .= "&nbsp;&nbsp;&nbsp;";
		$content .= "<a class='btn btn-danger' onclick='return confirm(\"Are you sure ?\");' href='<?php echo base_url()._INDEX; ?>".$vname."/delete/<?php echo $"."row->slug; ?>' >Delete</a></td>";
		$content .= "</td>";
		$content .= "</tr>\n";
