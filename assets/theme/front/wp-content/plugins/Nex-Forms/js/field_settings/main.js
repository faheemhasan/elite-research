...
function populate_logic(obj_id){
	...
	var output = '<button style="margin-bottom:15px; !important" class="add_condition btn btn-success form-control"><span class="fa fa-plus">&nbsp;</span> Add Condition</button>';
		...
		if(jQuery('#'+obj_id).hasClass('select') || jQuery('#'+obj_id).hasClass('radio-group'))
			output += '<div class="input_holder field_condition_template hidden"><div class="row"><div class="col-xs-5"><label>If the selected option is: <em class="setcondition"></em></label></div><div class="col-xs-3"><label>Action <em class="setaction"></em></label></div><div class="col-xs-3"><label>Target:<em class="targetname"></em></label></div><div class="col-xs-1"></div></div><div class="row"><div class="col-xs-5"><div class="">';
		else
			output += '<div class="input_holder field_condition_template hidden"><div class="row"><div class="col-xs-5"><label>If this field\'s value is: <em class="setcondition"></em></label></div><div class="col-xs-3"><label>Action <em class="setaction"></em></label></div><div class="col-xs-3"><label>Target:<em class="targetname"></em></label></div><div class="col-xs-1"></div></div><div class="row"><div class="col-xs-5"><div class="input-group">';
	...
	jQuery('.nex-forms-container .field_'+obj_id).each(
		function()
			...
			//if(!jQuery('#'+set_id))
				//{
				if(jQuery('#'+obj_id).hasClass('select') || jQuery('#'+obj_id).hasClass('radio-group'))
					output2 += '<div class="input_holder field_condition" id="'+ set_id +'"><div class="row"><div class="col-xs-5"><label>If the selected option is: </label></div><div class="col-xs-3"><label>Action: <em class="setaction"></em></label></div><div class="col-xs-3"><label>Target:<em class="targetname" data-target-id="'+ jQuery(this).attr('data-target') +'">'+ jQuery(this).attr('data-target-name') +'</em></label></div><div class="col-xs-1"></div></div><div class="row"><div class="col-xs-5"><div class="">';
				else
					output2 += '<div class="input_holder field_condition" id="'+ set_id +'"><div class="row"><div class="col-xs-5"><label>If this field\'s value is: <em class="setcondition">'+ jQuery(this).attr('data-condition') +'</em></label></div><div class="col-xs-3"><label>Action: <em class="setaction"></em></label></div><div class="col-xs-3"><label>Target:<em class="targetname" data-target-id="'+ jQuery(this).attr('data-target') +'">'+ jQuery(this).attr('data-target-name') +'</em></label></div><div class="col-xs-1"></div></div><div class="row"><div class="col-xs-5"><div class="input-group">';
...
(function($){
	...
	$(window).load(function() { // makes sure the whole site is loaded
	   ...
	   $('body').delay(1500).css({'overflow':'visible'});
		...
		$('.animate_form button').live('click',
			...
			$('#previewForm').on('hidden.bs.modal', function () {
			  $('.modal-body.ui-nex-forms-container').html('');
			})
	...
	var current_formcontainer_width = '';
		...
		jQuery('div.nex-forms-container label .the_label, div.nex-forms-container  label small').live('click',function(){ if(!jQuery('div.nex-forms-container').hasClass('selecting_conditional_target')) { jQuery('.nav a:first').trigger('click').tab('show'); } });
		jQuery('div.nex-forms-container div.input-inner .the_input_element, div.nex-forms-container #the-radios a, div.nex-forms-container a.ui-slider-handle, div.nex-forms-container .bootstrap-tagsinput').live('click',function(){  if(!jQuery('div.nex-forms-container').hasClass('selecting_conditional_target')) {jQuery('.nav li:eq(1) a').trigger('click').tab('show'); $(this).popover('hide'); setTimeout(function(){ jQuery('.nav li:eq(1) a').trigger('click'); },200) } });
		jQuery('div.nex-forms-container div.input-inner .help-block, div.nex-forms-container div.input-inner label .the_label .bs-tooltip').live('click',function(){  if(!jQuery('div.nex-forms-container').hasClass('selecting_conditional_target')) { jQuery('.nav li:eq(2) a').trigger('click').tab('show');} });
	
	jQuery('div.nex-forms-container div.form_object button.edit, div.nex-forms-container label#nexf_title, div.nex-forms-container label#title,div.nex-forms-container a.ui-slider-handle,div.nex-forms-container .bootstrap-tagsinput, div.nex-forms-container #the-radios a, div.nex-forms-container .grid .panel-heading, div.nex-forms-container div.input-inner .the_input_element, div.nex-forms-container div.input-inner .help-block').live('click',
		function(e)
			{
		...
		//POSITION
			$('.show-label button').click(
				function()
					...
					$(this).addClass('btn-primary');
					var get_label = $('#'+current_id).find('label#nexf_title');
					if(!get_label)
						get_label = $('#'+current_id).find('label#title');
