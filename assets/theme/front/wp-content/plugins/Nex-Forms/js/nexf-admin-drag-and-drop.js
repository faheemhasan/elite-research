...
function()
	...
	jQuery('.draggable_object .form-control').live('click',
		function()
			...
			jQuery( clone_element ).toggle('scale');
...
function setup_form_element(obj){
	...
	//if(!jQuery('#nex-forms-field-settings').hasClass('open'))
		//jQuery('div.nex-forms-container').find('div.field_settings').show();
	...
	setTimeout(
		function()
			...
			if(!obj.hasClass('dropped'))
				...
				obj.attr('id',set_Id);	
				jQuery('.nav a:first').trigger('click').tab('show');
				obj.find('button.edit').trigger('click');
...
function create_droppable(obj){
        ...
        the_draggable.draggable(
			{
			drag: function( event, ui ) { the_droppable.addClass('drag');},
			stop: function( event, ui ) { the_droppable.removeClass('drag'); setTimeout(function(){ jQuery('.col2 .admin-panel .panel-heading .btn.glyphicon-hand-down').trigger('click');},300 ); },
			stack  : '.draggable',
			revert : 'invalid', 
			tolerance: 'intersect',
