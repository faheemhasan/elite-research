...
jQuery(document).ready(function(){
    ...
    (function( bootstro, $, undefined ) {
        ...
        var defaults = {
            nextButtonText : 'Next <i class="fa fa-hand-o-right"></i>', //will be wrapped with button as below
            //nextButton : '<button class="btn btn-primary btn-xs bootstro-next-btn">Next &raquo;</button>',
            prevButtonText : '<i class="fa fa-hand-o-left"></i> Prev',
            //prevButton : '<button class="btn btn-primary btn-xs bootstro-prev-btn">&laquo; Prev</button>',
            finishButtonText : '<i class=" fa fa-thumbs-up"></i> OK',
            ...
            onStep : function(params){
				...
				if(activeIndex==17)
					jQuery('.col1 .form_field.text .draggable_object .form-control').trigger('click');
				if(activeIndex==18)
					jQuery('a.the-label').trigger('click');	
				if(activeIndex==19)
					jQuery('a.input-element').trigger('click');	
				if(activeIndex==20)
					jQuery('a.help-text').trigger('click');	
				if(activeIndex==21)
					jQuery('a.validation').trigger('click');	
				} ,
			//params = {idx : activeIndex, direction : [next|prev]}
        ...
        process_items = function(popover) 
        {
            ...
            $.each(popover, function(t,e){
                ...
                $.each(e, function(j, attr){
                    jQuery(e.selector).attr('data-bootstro-' + j, attr);
                });
                if (jQuery(e.selector).is(":visible"))
        ...
        get_popup = function(i)
        {
            ...
            p.template = '<div class="popover tutorial-popovers step_' + i +'" style="' + style + '"><div class="popover-arrow"></div><div class="popover-inner"><h3 class="tut-title popover-title"></h3><div class="popover-content"><p></p></div></div>' +
             '</div>';
        ...
        bootstro.destroy_popover = function(i)
        {
            ...
            else //destroy all
            {
                $elements.each(function(e){
                    ...
                    jQuery(e).popover('destroy').removeClass('bootstro-highlight');
                });
            }
            */
        };
        ...
        bootstro.stop = function()
        {
            ...
            bootstro.unbind();
            jQuery("div.bootstro-backdrop").remove();
        ...
        bootstro.go_to = function(idx) 
        {
            ...
            if (count != 0)
            {
                ...
                var docviewTop = jQuery(window).scrollTop();
                var top = Math.min(jQuery(".popover.in").offset().top, $el.offset().top);
                ...
                if (topDistance < settings.margin) //the element too up above
                    ...
                    'slow');
                else if(!is_entirely_visible(jQuery(".popover.in")) || !is_entirely_visible($el))
        ...
        bootstro._start = function(selector)
        {
            ...
            count  = $elements.size();
            if (count > 0 && jQuery('div.bootstro-backdrop').length === 0)
            {
                // Prevents multiple copies
                jQuery('<div class="bootstro-backdrop"></div>').appendTo('body');
                bootstro.bind();
                bootstro.go_to(0);
            }
        };
        ...
        bootstro.bind = function()
        {
            ...
            jQuery("html").on('click.bootstro', ".bootstro-next-btn", function(e){
                bootstro.next();
                e.preventDefault();
                return false;
            });
            
            jQuery("html").on('click.bootstro', ".bootstro-prev-btn", function(e){
                bootstro.prev();
                e.preventDefault();
                return false;
            });
            ...
            //end of show
            jQuery("html").on('click.bootstro', ".bootstro-finish-btn", function(e){
                e.preventDefault();
                bootstro.stop();
            });        
            ...
            {
                jQuery("html").on('click.bootstro', 'div.bootstro-backdrop', function(e){
                    if (jQuery(e.target).hasClass('bootstro-backdrop'))
                        bootstro.stop();
                });
            }
                
            //bind the key event
            jQuery(document).on('keydown.bootstro', function(e){
        ...
        {
            jQuery("html").unbind('click.bootstro');
            jQuery(document).unbind('keydown.bootstro');
        }
