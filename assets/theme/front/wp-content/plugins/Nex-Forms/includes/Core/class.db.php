...
//DB actions
add_action('wp_ajax_do_insert', array('IZC_Database','insert'));
...
if(!class_exists('IZC_Database'))
	...
	class IZC_Database{
		...
		public function direct_insert($table,$args,$prefix=''){
			...
			foreach($args as $key=>$val)
				{
				(!@strstr($key,str_replace('__','',$prefix)) && $key!='action' && $key!='edit_Id' && $key!='submit') ? $new_args[$prefix.$key] = $val : $new_args[$key] = $val;	
				}
			...
			foreach($fields as $field)
				{
				if(isset($new_args[$field->Field]))
					{
					$field_array[$field->Field] =  $new_args[$field->Field];
					}	
				}
			//print_r($field_array);
			$insert = $wpdb->insert ( $wpdb->prefix . $table, $field_array );
		...
		public function update(){
			...
			$update = $wpdb->update ( $wpdb->prefix . $_POST['table'], $field_array, array(	'Id' => $_POST['edit_Id']) );
			IZC_Functions::add_js_function('populate_list(\''.$_POST['fields'].'\',\''.$_POST['table'].'\',\''.$_POST['page'].'\')');
			IZC_Functions::print_message( 'updated' , 'Updated' );
			die();
		}
