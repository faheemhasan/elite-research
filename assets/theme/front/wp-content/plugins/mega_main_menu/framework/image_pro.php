 ...
 */
	if ( !class_exists( 'mm_image_pro' ) ) {
		...
		*/
			...
			public static function resized_image_src ( $original_image_uri = false, $width = false, $height = false, $crop = true ) {
		...
		*/
				$original_image_path = str_replace( $wp_upload_dir['baseurl'], $wp_upload_dir['basedir'], $original_image_uri );
		/* for better times
					...
					if ( $container == true ) {
						...
						if ( $cover == true ) {
							...
							if ( $cover == 'zoom' || ( is_array( $cover ) && in_array( 'zoom', $cover ) ) ) {
								$out .= mm_common::ntab(3) . '<a href="' . $src . '" data-rel="prettyPhoto" class="controls full_image' . ( ( $permalink != '' && in_array( 'link', $cover ) ) ? '' : ' single' ) . '">';
