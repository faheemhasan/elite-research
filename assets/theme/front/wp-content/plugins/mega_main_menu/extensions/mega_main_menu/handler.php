    ...
    function mmm_set_walkers ( $args ){
       ...
       $args = (array)$args;
        ...
        if ( ( is_array( $mega_menu_locations ) && in_array( $args['theme_location'], $mega_menu_locations ) ) || ( $indefinite_location_mode === true ) ) {
            ...
            ' icons-' . $mega_main_menu->get_option( $args['theme_location'] . '_first_level_icons_position', 'left' ) . 
           	' first-lvl-align-' . $mega_main_menu->get_option( $args['theme_location'] . '_first_level_item_align', 'left' ) . 
           	' first-lvl-separator-' . $mega_main_menu->get_option( $args['theme_location'] . '_first_level_separator', 'none' ) . 
           	' language_direction-' . $mega_main_menu->get_option( 'language_direction', 'ltr' ) . 
           	' direction-' . $mega_main_menu->get_option( $args['theme_location'] . '_direction', 'horizontal' ) . 
           	' responsive-' . ( ( is_array( $mega_main_menu->get_option( 'responsive_styles' ) ) && in_array( 'true', $mega_main_menu->get_option( 'responsive_styles' ) ) ) ? 'enable' : 'disable' ) . 
           	' mobile_minimized-' . ( ( ( is_array( $mega_main_menu->get_option( $args['theme_location'] . '_mobile_minimized' ) ) && in_array( 'true', $mega_main_menu->get_option( $args['theme_location'] . '_mobile_minimized' ) ) ) || ( $indefinite_location_mode === true ) ) ? 'enable' : 'disable' ) . 
//           	' fullwidth-' . ( ( is_array( $mega_main_menu->get_option( $args['theme_location'] . '_fullwidth_container' ) ) && in_array( 'true', $mega_main_menu->get_option( $args['theme_location'] . '_fullwidth_container' ) ) ) ? 'enable' : 'disable' ) . 
           	' dropdowns_animation-' . $mega_main_menu->get_option( $args['theme_location'] . '_dropdowns_animation', 'none' ) . 
            ...
            $args['depth'] = 5;
			$items_wrap = '';
			$data_sticky = ( (is_array( $mega_main_menu->get_option( $args['theme_location'] . '_sticky_status' ) ) && in_array( 'true', $mega_main_menu->get_option( $args['theme_location'] . '_sticky_status', array() ) ) ) /*|| ( $indefinite_location_mode === true )*/ ) 
				? ' data-sticky="1"' 
				: '';
			$data_sticky .= ( ( $mega_main_menu->get_option( $args['theme_location'] . '_sticky_offset' ) !== false && is_array( $mega_main_menu->get_option( $args['theme_location'] . '_sticky_status' ) ) && in_array( 'true', $mega_main_menu->get_option( $args['theme_location'] . '_sticky_status', array() ) ) ) || ( $indefinite_location_mode === true ) ) 
				? ' data-stickyoffset="' . $mega_main_menu->get_option( $args['theme_location'] . '_sticky_offset' ) . '"' 
				: '';
			if( ( is_array( $mega_main_menu->get_option( $args['theme_location'] . '_included_components' ) ) && in_array( 'company_logo', $mega_main_menu->get_option( $args['theme_location'] . '_included_components' ) ) ) && $mega_main_menu->get_option( 'logo_src' ) ) {
			...
			if ( has_filter( 'mmm_container_class' ) ) {
	            $container_class .= ' ' . esc_attr( apply_filters( 'mmm_container_class', '', $container_class ) );
			}
			...
			$items_wrap .= mm_common::ntab(1) . '<div class="fullwidth_container"></div><!-- class="fullwidth_container" -->';
			...
			$items_wrap .= mm_common::ntab(6) . $mega_main_menu->get_option( $args['theme_location'] . '_mobile_label', 'Menu' ) . ' &nbsp;';
